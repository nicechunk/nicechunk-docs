...
- Published one crawlable catalog and 31 physical English topic pages.
- Added 288 runtime topic locale files, including 256 non-English JSON editions, plus nine shell locale files.
- Added 109 optimized delivery images, 39 canonical game captures, model renders, and material references, plus 12 dated audit evidence files.
- Kept local-only production records and working image sources outside the publication mirror.
- Added the shared JSON-driven header runtime and removed the obsolete script that embedded translated navigation copy.
- Added the favicon and brand assets required to run the documentation tree as a standalone static site.
- Replaced the legacy repository overview with the current player-first field-guide architecture, static English delivery contract, page-scoped locale model, and Nginx hosting guidance.

### Validation

- `npm run check:docs-i18n`
- Static English generation parity and no-JavaScript content checks
- Local browser checks for English, Simplified Chinese, German, and a 390-pixel mobile viewport
- Staged whitespace, JSON syntax, secret pattern, file-size, and Han character policy checks

### Source Anchors

- Main working-tree Docs source normalization: `4e3ae71`
- Main working-tree Docs synchronization tooling: `ac8abb8`
- Split repository baseline: `e90a1ee`
...
NiceChunk documentation and design notes.
