...
| `npm run release:evidence` | Main and split repository provenance | Current commit, author, branch, dirty status, upstream, split remote/branch/author policy matches, required review files, and expected validation commands are emitted as JSON | Use after validation to capture release evidence. |
