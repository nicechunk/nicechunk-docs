...
This matrix maps validation commands to the project risks they cover. It is intended for maintainers and external reviewers that need reproducible evidence rather than a loose claim that the repository is healthy.
...
Use the review ownership document and threat model to identify the trust boundary, protected asset, and extra evidence required for high-risk changes.
...
- Public review flow for external reviewers.
- Root README, changelog, security, contribution, support, and conduct files are included in public-copy checks.
