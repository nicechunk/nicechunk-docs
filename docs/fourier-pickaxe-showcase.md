...
Fourier Voxel is an independent GPU-oriented NiceChunk browser program for compact voxel asset functions. This document gives external reviewers enough context to assess the `fourier-voxel/` source without requiring this environment to run the GPU workload.
...
Use the following card as the safe non-GPU presentation for GitHub, external review, or investor-facing technical screening. The same reviewer-facing packet is maintained in `docs/fourier-pickaxe-static-display.md`. It intentionally avoids screenshots, performance numbers, or visual-fidelity claims because those require GPU hardware.
